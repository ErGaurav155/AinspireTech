// app/api/subscriptions/insta/cancel/route.ts
...
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json(
      {
        success: true,
        message: "Insta subscription cancelled",
      },
      { status: 200 }
    );
  } catch (error: any) {
    console.error("Insta cancellation error:", error);
    return NextResponse.json(
      {
        success: false,
        message: error.message || "Failed to cancel Insta subscription",
      },
      { status: 500 }
    );
  }
