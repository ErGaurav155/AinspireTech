...
export default function TokenDashboard() {
  ...
  const fetchTokenData = async () => {
    try {
      ...
      const [balanceRes, usageRes] = await Promise.all([
        ...
        fetch("/api/tokens/usage?period=month"),
      ]);
      ...
      if (balanceRes.ok && usageRes.ok) {
        ...
        setTokenStats(balanceData.data);
  ...
  const handleResetFreeTokens = async () => {
    try {
      const response = await fetch("/api/tokens/reset-free", {
        method: "POST",
      });
      ...
      toast({
        title: "Error",
        description: "Failed to reset free tokens",
        variant: "destructive",
      });
    }
  };
