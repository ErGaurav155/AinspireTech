...
export default function AccountPage({
  ...
  }, [account?.accountId, fetchTemplates]);
    ...
    async (accountId: string) => {
      ...
      try {
        ...
        if (cachedData) {
          try {
            ...
            if (
              ...
              const cachedAccount =
                data.find((account: any) => account.id === accountId) || null;

              setIsStale(!isCacheValid());
              if (cachedAccount?.accountId) {
        ...
        const validAccounts = completeAccounts.filter(
          (account): account is any => account !== null
        );

        if (validAccounts.length > 0) {
          const infoAccount =
            validAccounts.find((account: any) => account.id === accountId) ||
            null;
    ...
    try {
      ...
      if (userId) {
        try {
          ...
          if (response.ok) {
            const data = await response.json();
            recentActivity = data?.replyLogs || [];
          }
        } catch (logError) {
          console.error("Error fetching reply logs:", logError);
        }
      }
  ...
  const handleCreateTemplate = useCallback(async () => {
    if (!userId || !account?.accountId) return;
