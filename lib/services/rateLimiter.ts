...
export async function canMakeCall(
  ...
  if (
    ...
    const remainingBlockTime = rateLimit.blockedUntil.getTime() - now.getTime();
    ...
    await RateLimitLog.create({
      ...
      action,
      timestamp: now,
      remainingCalls: CALLS_PER_HOUR - rateLimit.calls,
      status: "RATE_LIMITED",
  ...
  if (rateLimit.calls >= BLOCK_THRESHOLD) {
    rateLimit.isBlocked = true;
    rateLimit.blockedUntil = new Date(now.getTime() + BLOCK_DURATION_MS);
    await rateLimit.save();
  ...
  if (rateLimit.calls >= CALLS_PER_HOUR) {
    await RateLimitLog.create({
      accountId,
      userId,
      action,
  ...
  const remainingCalls = CALLS_PER_HOUR - rateLimit.calls;
...
export async function getAccountStatus(accountId: string): Promise<{
  ...
  return {
    ...
    remaining: Math.max(0, CALLS_PER_HOUR - rateLimit.calls),
 ...
 */
export async function resetAccountRateLimit(accountId: string): Promise<void> {
  await connectToDatabase();
